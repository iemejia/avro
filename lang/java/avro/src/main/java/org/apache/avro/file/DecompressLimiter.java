...
public final class DecompressLimiter {
  ...
  public static final String MAX_LENGTH_PROPERTY = "org.apache.avro.limits.decompress.maxLength";
  ...
  /**
   * Throws an {@link AvroRuntimeException} if {@code size} exceeds the configured
   * maximum. Suitable for codecs that know the output size before decompression
   * (e.g.&nbsp;Snappy).
   *
  ...
  private static void checkAgainst(long size, long limit) {
    if (size > limit) {
      throw new AvroRuntimeException("Decompressed size " + size + " (bytes) exceeds maximum allowed size " + limit
