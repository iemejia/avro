...
/**
 * Tests that all Trevni compression codecs enforce the decompression size
 * limit, preventing decompression bomb (zip bomb) denial-of-service attacks.
 *
 * <p>
...
public class TestDecompressionBomb {
  ...
  /**
   * All Trevni codecs that perform actual compression/decompression. The null
   * codec is excluded because it passes data through unchanged.
   */
  ...
  /**
   * Verify that decompression throws {@link TrevniRuntimeException} when the
  ...
  void decompressExceedingLimitThrows(String codecName) throws IOException {
    ...
    TrevniRuntimeException ex = assertThrows(TrevniRuntimeException.class, () -> codec.decompress(compressed));
